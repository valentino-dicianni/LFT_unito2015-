...
/*  Modificare il programma dell' esercizio precedente in modo da tradurre programmi ben tipati scritti nel frammento del linguaggio P che
 permette non solo il comando print e le espressione aritmetico-logiche ma anche l’utilizzo di variabili. Più precisamente, il frammento 
 permette (1) la dichiarazione di variabili, (2) l’assegnazione di valori di espressioni a variabili, e (3) l’utilizzo di variabili in espressioni.
 */
...
/* Creo una variabile statica address che mi tenga conto degli indirizzi quando vado a riempire la symbolTable con le nuove variabili:
	...
	*/
    ...
    /*
	<stat> ::= ID :=<orE> {if <id>.type != <or_E>.tipo ERROR( ) ; emit (istore address (ID.lexeme))}
			| print (<orE>) {emit(istruzione di stampa)}
			| begin <statlist> end 
	*/
    ...
    private void stat(CodeGenerator code, SymbolTable table) {
        ...
        switch(look.tag){
            case Tag.ID:
                ...
                // ... type checking ...
                if(ID_type != orE_type) error("ERRORE stat");
