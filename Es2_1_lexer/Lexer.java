...
public class Lexer {
	...
	/* Si scriva in Java un analizzatore lessicale che legga da tastiera comandi scritti in questo linguaggio e per ciascuna comando stampi una sequenza di token. 
		• I token che corrispondono a numeri prenderanno la formahNUM,valorei, dove valore `e un intero non negativo. 
		Ad esempio, il token che corrisponde al numero 12 sarà <NUM,12>

		• I token che corrispondono agli identiﬁcatori prenderanno la forma <ID,“lessema”>. 
		
		• I token che corrispondono aglielementi della sintassi che consistono di un solo carattere(ad esempio, ’(’, ’+’ e ’:’) prenderanno la forma <nome> ,dove il nome è 
		il codice ASCII del carattere.  
		
		• I token che corrispondono agli elementi della sintassi che consistono di più caratteri (ad esempio, &&, <>, print, integer e :=)
		prenderanno la forma <nome,“lessema”>.
		...
		switch (peek) {	/* gestione della casistica del carattere peek in imput*/
			...
			case '&':		/* la '&' viene gestita a parte perchè  è diversa da && */
				...
				if (peek == '&') {
					...
					System.err.println("Erroneous character" + " after & : " + peek );	/* se la prima '&' non è seguita da una seconda, errore.*/
					return null;		/* viene ritornato NULL */ 
				}
...
public static void main(String[] args) {
	Lexer lex = new Lexer();		/* creo nuovo lexer */
	Token tok;
	do {
